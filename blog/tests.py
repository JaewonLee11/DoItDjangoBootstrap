...
class TestView(TestCase):
    ...
    def test_update_post(self):
        ...
        self.assertIn('파이썬 공부; python', tags_str_input.attrs['value'])
    ...
    def test_comment_update(self):
        ...
        self.assertTrue(comment_001_update_btn.text)
        self.assertEqual(comment_001_update_btn.attrs['href'], 'blog/update-comment/1/')

        self.assertIn('edit', comment_001_update_btn.text)
        self.assertEqual(comment_001_update_btn.attrs['href'], '/blog/update-comment/1/')

        response = self.client.get('/blog/update-comment/1/')
        self.assertEqual(response.status_code, 200)
        soup=BeautifulSoup(response.content, 'html.parser')

        self.assertEqual('Edit Comment - Blog', soup.title.text)
        update_comment_form = soup.find('form', id='comment_form')
        content_textarea = update_comment_form.find('textarea', id='id_content')
        self.assertIn(self.comment_001.content, content_textarea.text)

        response = self.client.post(
            f'/blog/update-comment/1/{self.comment_001.pk}/',
            {
        ...
        self.assertEqual(response.status_code, 200)
        soup=BeautifulSoup(response.content, 'html.parser')
        comment_001_div = soup.find('div', id='comment-001')
    ...
    def test_delete_comment(self):
        ...
        comment_area=soup.find('div', id='comment-area')
        self.assertFalse(comment_area.find('a', id='comment-1-delete-btn'))
        comment_002_delete_btn=comment_area.find('a', id='comment-2-delete-btn')
        self.assertIn('delete', comment_002_delete_btn.text)
        self.assertEqual(comment_002_delete_btn.attrs['data-target'], '#deleteCommentModal-2')

        delete_comment_002=soup.find('div', id='deleteCommentModal-2')
        self.assertIn('Are you Sure?', delete_comment_002.text)
        really_delete_btn_002 = delete_comment_modal_002.find('a')
        self.assertIn('Delete', really_delete_btn_002.text)
        self.assertEqual(really_delete_btn_002.attrs['href'], 'blog/delete-comment/2/')

        response = self.client.get('/blog/delete-comment/2/', follow=True)
        self.assertEqual(response.status_code, 200)
        soup=BeautifulSoup(response.content, 'html.parser')
        self.assertIn(self.post_001.title, soup.title.text)
        comment_area=soup.find('div', id='comment-area')
